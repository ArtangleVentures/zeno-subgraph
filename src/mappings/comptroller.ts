...
function createComptroller(address: Address): Comptroller {
  ...
  // Fetch the priceOracle
  let oracleResult = contract.try_oracle()
  if (!oracleResult.reverted) {
    comptroller.priceOracle = oracleResult.value
  } else {
    comptroller.priceOracle = Address.zero()
  }

  // Fetch closeFactor
  let closeFactorResult = contract.try_closeFactorMantissa()
  if (!closeFactorResult.reverted) {
    comptroller.closeFactor = closeFactorResult.value
  } else {
    comptroller.closeFactor = BigInt.zero()

  }

  // Fetch liquidationIncentive
  let liquidationIncentiveResult = contract.try_liquidationIncentiveMantissa()
  if (!liquidationIncentiveResult.reverted) {
    comptroller.liquidationIncentive = liquidationIncentiveResult.value
  } else {
    comptroller.liquidationIncentive = BigInt.zero()
  }

  // Fetch pauseGuardian
  let pauseGuardianResult = contract.try_pauseGuardian()
  if (!pauseGuardianResult.reverted) {
    comptroller.pauseGuardian = pauseGuardianResult.value
  } else {
    comptroller.pauseGuardian = Address.zero()
  }

  // Fetch borrowCapGuardian
  let borrowCapGuardianResult = contract.try_borrowCapGuardian()
  if (!borrowCapGuardianResult.reverted) {
    comptroller.borrowCapGuardian = borrowCapGuardianResult.value
  } else {
    comptroller.borrowCapGuardian = Address.zero()
  }

  // Fetch supplyCapGuardian
  let supplyCapGuardianResult = contract.try_supplyCapGuardian()
  if (!supplyCapGuardianResult.reverted) {
    comptroller.supplyCapGuardian = supplyCapGuardianResult.value
  } else {
    comptroller.supplyCapGuardian = Address.zero()
  }

  // Fetch liquidityMining
  let liquidityMiningResult = contract.try_liquidityMining()
  if (!liquidityMiningResult.reverted) {
    comptroller.liquidityMining = liquidityMiningResult.value
  } else {
    comptroller.liquidityMining = Address.zero()
  }

  // Set paused states to default values
  comptroller.seizeGuardianPaused = false
  comptroller.transferGuardianPaused = false
